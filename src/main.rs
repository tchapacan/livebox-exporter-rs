...
async fn main() {
    ...
    env::set_var("RUST_LOG", format!("folder_size={},livebox-exporter-rs={}", log_level, log_level));
...
async fn render_livebox_metrics(request: Request<Body>, _options: Arc<MyOptions>, matches: ArgMatches) -> Result<String, Box<dyn Error + Send + Sync>> {
    trace!("In our render_prometheus(request == {:?}, options == {:?})", request, _options);
    ...
    let rendered_metrics = vec![
        render_livebox_info_metric(&status, "livebox_infos_status", "Livebox general status", |s| if s.device_status == "Up" { 1 } else { 0 }),
        render_livebox_info_metric(&status, "livebox_infos_uptime", "Livebox uptime", |s| s.up_time.try_into().unwrap()),
        render_livebox_info_metric(&status, "livebox_infos_reboot", "Livebox count of reboots", |s| s.number_of_reboots.try_into().unwrap()),
        render_livebox_status_metric(&wan, "livebox_wan_status", "wan"),
        render_livebox_status_metric(&wan, "livebox_link_status", "link"),
        render_livebox_interface_metric(&metrics, "livebox_interface_bytes_rx", "Livebox interface bytes RX", |e: &TrafficData| e.rx_counter.try_into().unwrap(), "rx"),
        render_livebox_interface_metric(&metrics, "livebox_interface_bytes_tx", "Livebox interface bytes TX", |e: &TrafficData| e.tx_counter.try_into().unwrap(), "tx"),        
        render_livebox_devices_metric(&devices, "livebox_device_status", "Livebox connected devices status", |d| if d.active { 1 } else { 0 }),
    ];
    ...
    F: FnOnce(&Status) -> usize,
{
    create_metric(name, help)
        .render_and_append_instance(&PrometheusInstance::new()
...
fn render_livebox_status_metric(wan_config: &WANConfiguration, name: &str, port: &str) -> String {
    create_metric(name, &format!("Livebox {} status", port))
        .render_and_append_instance(&PrometheusInstance::new()
            ...
            .with_value(if port == "wan" { wan_config.wan_state == "up" } else { wan_config.link_state == "up" } as usize)
            .with_current_timestamp()
            .expect("Error getting the current UNIX epoch"))
        .render()
}

fn render_livebox_interface_metric<F>(metrics: &[Metrics], name: &str, help: &str, value_fn: F, direction: &str) -> String
where
    ...
    for metric in metrics {
        for (interface_name, interface_data) in metric.status.iter() {
            for entry in &interface_data.traffic {
                rendered_metrics.render_and_append_instance(&PrometheusInstance::new()
...
fn render_livebox_devices_metric<F>(devices: &[Device], name: &str, help: &str, value_fn: F) -> String
where
    F: Fn(&Device) -> usize,
{
    let mut rendered_metrics = create_metric(name, help);
    for device in devices {
        rendered_metrics.render_and_append_instance(
            &PrometheusInstance::new()
                ...
                .with_label("ip_address", &*device.ip_address.clone().unwrap_or("".to_string()))
...
mod tests {
    ...
    fn test_parse_args_default() {
        ...
        assert_eq!(matches.get_one::<String>("port"), Some(&String::from("9100")));
        assert_eq!(matches.get_one::<String>("address"), Some(&String::from("0.0.0.0")));
        assert_eq!(matches.get_count("verbose"), 0);
        assert_eq!(matches.get_one::<String>("password"), Some(&String::from("mypassword")));
    }

    #[test]
    fn test_parse_args_custom() {
        let args = vec!["livebox-exporter-rs", "-p", "1234", "--listen", "127.0.0.1", "-vvv", "-P", "mypassword"];
        let matches = parse_args(args);
        assert_eq!(matches.get_one::<String>("port"), Some(&String::from("1234")));
        assert_eq!(matches.get_one::<String>("address"), Some(&String::from("127.0.0.1")));
    ...
    fn test_render_livebox_info_metric() {
        ...
        let result = render_livebox_info_metric(
            &status,
            "test_name",
            "test_help",
            |s| if s.device_status == "Up" { 1 } else { 0 },
        );
        let expected_output_with_timestamp = expected_output.replace("TIMESTAMP_PLACEHOLDER", &result.split_whitespace().last().unwrap());
    ...
    fn test_render_livebox_devices_metric() {
        ...
        let result = render_livebox_devices_metric(&devices, "test_name", "test_help", |d| if d.active { 1 } else { 0 });
        let expected_output_with_timestamp = expected_output.replace(
            "TIMESTAMP_PLACEHOLDER",
            &result.split_whitespace().last().unwrap(),
        );
